...
const runner = Workspace.init(
  ...
  async ({ root }) => {
    ...
    return { beyond, person_b };
  }
);

runner.test("premint", async (t, { root, beyond, person_b }) => {
  // Owner mints one before presale
  await userMintsNFTs(t, root, beyond, 1);
  // PersonB's mint fails before presale
  await userMintsNFTs(t, person_b, beyond, 1, true);
